...
export async function searchShops(req: Request, res: Response) {
  try {
    const { city, district, name } = req.query as any;
    const where: any = {};
...
export async function addShopImage(req: Request, res: Response) {
  try {
    ...
    if (!shop) return res.status(404).json({ message: 'Shop not found' });
    if (shop.ownerId !== user.userId && user.role !== 'ADMIN') return res.status(403).json({ message: 'Forbidden' });
