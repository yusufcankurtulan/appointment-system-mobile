...
class ShopsRepository {
  Future<List<ShopModel>> getNearby(double lat, double lng, {double radius = 10}) async {
    final resp = await dio.get('/shops/nearby', queryParameters: {'lat': lat, 'lng': lng, 'radius': radius});
    final data = resp.data as List<dynamic>;
    return data.map((e) => ShopModel.fromJson(e as Map<String, dynamic>)).toList();
  }

  Future<List<ShopModel>> search({String? city, String? district, String? name}) async {
